...
const DropdownSection = <T extends object>(props: DropdownSectionProps<T>) => {
    return (
        <Primitive.Section className="after:block after:h-[5px] after:content-[''] first:-mt-[5px]">
            <Primitive.Header className='dsh mb-0.5 px-2 text-sm text-muted-foreground'>{props.title}</Primitive.Header>
            <Primitive.Collection items={props.items}>{props.children}</Primitive.Collection>
        </Primitive.Section>
    )
}
