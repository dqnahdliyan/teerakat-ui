...
interface DateFieldProps<T extends Primitive.DateValue> extends Primitive.DateFieldProps<T> {
    label?: string
    description?: string
...
const DateField = <T extends Primitive.DateValue>({
    ...
    return (
        <Primitive.DateField {...props} className={cn('flex flex-col gap-1', props.className)}>
...
const DateInput = (props: Omit<Primitive.DateInputProps, 'children'>) => {
    return (
        <Primitive.DateInput
            ...
            {...props}
        >
            {(segment) => <Primitive.DateSegment segment={segment} className={segmentStyles} />}
        </Primitive.DateInput>
    )
}
