...
const SheetOverlay = ({ className, isDismissable = true, ...props }: Primitive.ModalOverlayProps) => (
...
const SheetContent = (props: SheetContentProps) => {
    ...
    return (
        <Primitive.Modal className={cn(sheetVariants({ side, isStack }), className)} {...props}>
            <Dialog aria-label='Sheet' role={role} className='h-full outline-none'>
                {(values) => (
                    <>
                        {typeof children === 'function' ? children(values) : children}
                        {closeButton && <CloseButtonIndicator className='right-2.5 top-2.5' close={values.close} />}
                    </>
                )}
            </Dialog>
        </Primitive.Modal>
