...
const Slider: SliderComponent = React.forwardRef<React.ElementRef<typeof Primitive.Slider>, SliderProps>(
...
const SliderRoot = React.forwardRef((props: SliderRootProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    return (
        <Primitive.TextContext.Provider value={{ slots: { description: { id: descriptionId } } }}>
            <Primitive.Slider
                ref={ref}
                aria-describedby={descriptionId}
                {...props}
                className={Primitive.composeRenderProps(props.className, (className) => root({ className }))}
            />
...
const SliderTrack = (props: SliderTrackProps) => {
    ...
    return (
        <Primitive.SliderTrack
            {...props}
            className={Primitive.composeRenderProps(props.className, (className) => track({ className }))}
        />
    )
}
...
const SliderFiller = (props: SliderFillerProps) => {
    const { orientation, getThumbPercent, values } = React.useContext(Primitive.SliderStateContext)
    return (
        <div
            {...props}
            style={
...
const SliderThumb = (props: SliderThumbProps) => {
    return (
        <Primitive.SliderThumb
            {...props}
            className={Primitive.composeRenderProps(props.className, (className) => thumb({ className }))}
        />
    )
}
...
const SliderValueLabel = (props: LabelProps) => {
    ...
    return (
        <Primitive.SliderOutput
            {...props}
            className={Primitive.composeRenderProps(props.className, (className) => valueLabel({ className }))}
        >
