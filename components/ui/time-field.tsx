...
export interface TimeFieldProps<T extends Primitive.TimeValue> extends Primitive.TimeFieldProps<T> {
    label?: string
    description?: string
...
export function TimeField<T extends Primitive.TimeValue>({
    ...
    return (
        <Primitive.TimeField {...props} className={cn('flex flex-col gap-1', props.className)}>
            <Label>{label}</Label>
            <DateInput
                className={cn(
