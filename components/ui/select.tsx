...
interface SelectProps<T extends object> extends Omit<Primitive.SelectProps<T>, 'children'> {
    label?: string
    description?: string
...
function Select<T extends object>({ label, description, errorMessage, children, items, ...props }: SelectProps<T>) {
    return (
        <Primitive.Select {...props} className={cn('group flex w-full flex-col gap-1 outline-none', props.className)}>
